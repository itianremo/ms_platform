...
class _ProfileCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final adminBlue = const Color(0xFF003366);
    final orange = Colors.amber; // "the orange color already used"
...
class _PagedProfileList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return FutureBuilder<List<Recommendation>>(
      ...
      builder: (context, snapshot) {
        ...
        if (data.isEmpty) return const Center(child: Text("No anyone yet."));
        ...
        if (isGrid) {
          return GridView.builder(
            ...
            itemBuilder: (context, i) {
              final builder = itemBuilderWithIndex;
              if (builder != null) {
                return builder(context, data[i], i);
              }
              return itemBuilder(context, data[i]);
            },
          );
        } else {
  ...
  Future<List<Recommendation>> _fetch(WidgetRef ref) async {
    try {
      const userId = "22222222-2222-2222-2222-222222222222";
      return await fetcher(ref.read(authClientProvider), userId);
    } catch (e) {
      return [];
    }
  }
