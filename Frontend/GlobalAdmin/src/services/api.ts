...
const api = axios.create({
    baseURL: '/api', // Proxied via Nginx to Gateway (rewritten)
    headers: {
    ...
    async (error) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
            ...
            try {
                ...
                // Note: baseURL is '/api' so full path is needed if using raw axios, or relative if same host.
            ...
            import('sonner').then(({ toast }) => {
                toast.error("Please calm down! Wait 5 seconds before trying again.", { duration: 5000 });
            });
        }
